...
public class GitLabHTTPRequestor {
    ...
    public <T> Paged<T> toPaged(String tailAPIUrl, Class<T[]> type) throws IOException {
        method("GET");
        ...
        try {
            ...
            try {
                T[] res = parse(connection, type, null);
                return new PagedImpl<>(root, type, Arrays.asList(res), getIntegerHeaderField(connection, "X-Total"), getIntegerHeaderField(connection, "X-Total-Pages"),
                        getIntegerHeaderField(connection, "X-Per-Page"), getIntegerHeaderField(connection, "X-Page"), getIntegerHeaderField(connection, "X-Next-Page"),
                        getIntegerHeaderField(connection, "X-Prev-Page"), connection.getHeaderField("Link"));
            } catch (IOException e) {
                handleAPIError(e, connection);
            }
    ...
    private Integer getIntegerHeaderField(HttpURLConnection connection, String key) {
    ...
    private <T> T parse(HttpURLConnection connection, Class<T> type, T instance) throws IOException {
        ...
        try {
            if (byte[].class == type) {
                byte[] bs = IOUtils.toByteArray(wrapStream(connection, connection.getInputStream()));
                return type.cast(bs);
            }
